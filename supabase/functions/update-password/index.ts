...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!userId || !newPassword) return new Response(JSON.stringify({ error: 'userId et newPassword requis' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })

    const { error } = await supabaseAdmin.auth.admin.updateUserById(userId, { password: newPassword })
    if (error) return new Response(JSON.stringify({ error: error.message }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })

    // Update plain_password in profile
    await supabaseAdmin.from('profiles').update({ plain_password: newPassword }).eq('user_id', userId)
